...
# --- Extract System A/B table (rows 8–13, columns 2–4) ---
data_ab = df.iloc[8:14, 2:5].copy()
data_ab.columns = ['Loading Category', 'Hydraulically Bound Base (mm)', 'Coarse Graded Material (mm)']
data_ab.insert(1, 'Block/Laying Course (mm)', '80/50')  # Fixed value
data_ab = data_ab.dropna().reset_index(drop=True)

# --- Extract System C table (rows 20–25, columns 2–5) ---
...
# Manually assign loading categories
loading_categories = [
    "A/domestic",
    "B/car parking",
    "C/pedestrian",
...
data_c.insert(0, 'Loading Category', loading_categories)
...
category = st.selectbox("Select Loading Category", loading_categories)
