...
if area_m2 > 0:
    try:
        ...
        # Additional Carbon for Material Layers (Embodied + Laying)
        # Constants (kg CO₂e per m³)
        carbon_block_layer = 120    # 80mm block + 50mm bedding sand (fixed 130mm)
        carbon_hbm = 45             # Hydraulically Bound Material (HBCGA)
        carbon_cga = 8              # Coarse Graded Aggregate
        carbon_capping = 5          # 6F4 / 6F5 Capping

        # Convert mm to m for thickness
        hbb_m = hbb / 1000
        cgm_m = cgm / 1000
        cap_m = cap / 1000

        # Volume per layer
        block_vol = area_m2 * 0.13  # 130mm fixed
        ...
        # Total material-based carbon
        total_material_carbon = carbon_block + carbon_hbb + carbon_cgm + carbon_cap

        st.markdown("### Build-Up Carbon Estimates")
        st.write(f"**Block + Bedding Sand:** {carbon_block:.0f} kg CO₂e")
        st.write(f"**Hydraulically Bound Material:** {carbon_hbb:.0f} kg CO₂e")
        st.write(f"**Coarse Graded Aggregate:** {carbon_cgm:.0f} kg CO₂e")
        st.write(f"**Capping Layer:** {carbon_cap:.0f} kg CO₂e")
        st.markdown(f"**Total Build-Up Carbon:** {total_material_carbon:.0f} kg CO₂e")
