...
if not match.empty:
    ...
    # --- Build-up Diagram ---
    st.subheader("Build-up Diagram")

    # Parse Block/Laying Course
    block, laying = map(int, row['Block/Laying Course (mm)'].split('/'))

    layers = [
        ("Block Pavers", block),
        ("Laying Course", laying),
        ("H.B. Base", int(row['Hydraulically Bound Base (mm)'])),
        ("CGM Sub-base", int(row['Coarse Graded Material (mm)'])),
        ("Capping Layer", int(row['Capping Layer (mm)']))
    ]
