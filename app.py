...
sheet_ab = pd.read_excel(excel_file, sheet_name="System AB", skiprows=8, nrows=6)
sheet_c = pd.read_excel(excel_file, sheet_name="System C", skiprows=8, nrows=6)

# Rename columns
columns_ab = ['Loading Category', 'Block/Laying Course (mm)', 'Hydraulically Bound Base (mm)', 'Coarse Graded Material (mm)', 'Capping Layer (mm)']
columns_c = columns_ab
sheet_ab.columns = columns_ab
sheet_c.columns = columns_c
