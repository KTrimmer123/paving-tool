...
image_ab = Image.open(os.path.join("images", "System A or B.png"))
image_c = Image.open(os.path.join("images", "System C.png"))
...
# Read full sheet
df = pd.read_excel(excel_file, sheet_name="Sheet1", skiprows=8, nrows=12)
...
# Select correct row based on system and loading category
row_map = {
    "A/domestic": 0,
    "B/car parking": 1,
    "C/pedestrian": 2,
...
if "System C" in system:
    row_index = row_map[category] + 6  # bottom half of table
else:
    row_index = row_map[category]      # top half of table
